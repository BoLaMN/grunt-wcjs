...
module.exports = (grunt) ->
  ...
  [
    ...
    grunt.registerTask 'wcjs:electron:' + dist, 'Download electron',  ->
      done = @async()
      ...
      if runtime isnt 'electron'
        done()
        return

      downloadDir = path.join os.tmpdir(), 'grunt-wcjs', 'electron'
      symbols = false
      rebuild = false
      apm = getApmPath(dist)
      ...
      if getAtomShellVersion(versionDownloadDir)?
        ...
        copyDirectory(versionDownloadDir, outputDir, true)
  ...
  grunt.registerTask 'wcjs:electron', [
    'wcjs:electron:darwin64'
    'wcjs:electron:linux32'
    'wcjs:electron:linux64'
    'wcjs:electron:win32'
  ]
